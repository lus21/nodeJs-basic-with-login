...
passport.use('local.signup', new LocalStrategy.Strategy({
    ...
    req.checkBody('email').notEmpty().isEmail().withMessage('Email is invalid.');
    req.checkBody('password').notEmpty().isLength({min:4}).withMessage('Password is required and must have min 4 characters.');
    req.checkBody('name').notEmpty().withMessage('Name is is required');
    req.checkBody('confirmPassword').notEmpty().isLength({min:4}).withMessage('Confirm password is required and must have min 4 characters.');
    ...
    UsersModel.findOne({'email': email})
        .then( (user) => {
            ...
            if (user) {
                return callback(null, false, {message: 'Please enter another email. This one is already in use.'})
            }
...
passport.use('local-sign-in', new LocalStrategy({
    ...
    function(req, email, password, done) { // callback with email and password from our form
        ...
        UsersModel.findOne({ 'email' :  email }, function(err, user) {
            ...
            if (!user)
                return done(null, false, req.flash('error', ['Oops! Wrong credentials.'])); // req.flash is the way to set flashdata using connect-flash

            // if the user is found but the password is wrong
            if (!user.isValidPassword(password))
                return done(null, false, req.flash('error', ['Oops! Wrong credentials.'])); // create the messages and save it to session as flashdata
